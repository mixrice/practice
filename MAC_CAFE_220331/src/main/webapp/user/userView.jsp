...
<section id="joinformArea">
	<form action="userUpdate.usr" method="post" name="f">
		<!-- 처음 가입이면 무조건 Normal 등급 -->
		<input type="hidden" name="u_grade" value="Normal">
		<table>
			<tr>
				<th colspan="2">
					<h2>[회원정보]</h2>
					<h3>회원님의 등급은 ${user.u_grade }입니다.</h3>
					<h4>(※ 기본등급 : Normal, 지난달 총 주문금액 100,000원 이상 VIP로 업그레이드됩니다.)</h4>
				</th>
			</tr>
			<tr>
				<th>아이디</th>
				<td>
					<input type="text" name="u_id" id="u_id" value="${user.u_id}" size="40" maxlength="12" readonly="readonly">
				</td>
			</tr>
			<tr>
				<th>이름</th>
				<td>
					<input type="text" name="u_name" id="u_name" value="${user.u_name}" size="40"  placeholder="한글또는 영문만 입력하세요.(특수문자제외)" required="required">
				</td>
			</tr>
			<tr>
				<th>이메일</th>
				<td>
					<input type="text" name="u_email" id="u_email" value="${user.u_email}" size="40"  placeholder="(ex)aaa@naver.com" required>
				</td>
			</tr>
			<tr>
				<th>휴대전화</th>
				<td>
					<input type="text" name="u_call" id="u_call" value="${user.u_call}" size="40"  placeholder="(-)없이 숫자만 입력하세요." required>
				</td>
			</tr>
			<tr>
				<th>주소</th>
				<td>
					<input type="text" name="postcode" id="postcode" value="${addr.postcode}" size="24" placeholder="우편번호만 입력" required="required">
					<input type="button" value="우편번호 찾기" onclick="findAddr();" required="required"><br>
					
					<input type="text" name="address1" id="address1" value="${addr.address1}" size="40" placeholder="주소" required="required"><br>
					<input type="text" name="address2" id="address2" value="${addr.address2}" size="40" placeholder="상세주소" required="required"><br>
				</td>
			</tr>
			<tr>
				<th colspan="2">
					<input type="submit" value="수정하기" onclick="check();">
					<a href="userDelete.usr?u_id=${user.u_id }">[회원탈퇴]</a>
				</th>
			</tr>
		</table>
	</form>
