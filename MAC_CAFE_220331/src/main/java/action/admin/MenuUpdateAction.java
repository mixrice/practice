...
import svc.admin.MenuUpdateService;
...
public class MenuUpdateAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 파라미터로 전송된 m_id로 제품의 상세정보를 얻어와 신메뉴인지 아닌지를 판별한다. (※ 신메뉴 추가할떄만 함)
		...
		*/	
			...
			MenuUpdateService menuUpdateService = new MenuUpdateService(); // 관리자만 신메뉴를 등록할 수 있으므로 svc.admin 클래스에 넣어야함
			boolean isUpdateMenuSuccess = menuUpdateService.updateMenu(newMenu);
			
			if(isUpdateMenuSuccess) { // 메뉴등록에 성공했으면
				...
				out.println("<script>");
				out.println("alert('메뉴수정에 실패했습니다.');");
